...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $modeleChev = $_POST['chevaletModel'] ?? null;
    ...
    $nomEpoux = $_POST['nomEpoux'] ?? null;
    $prenomEpouse = $_POST['prenomEpouse'] ?? null;
    $nomEpouse = $_POST['nomEpouse'] ?? null;
    $nomFamilleEpoux = $_POST['nomFamilleEpoux'] ?? null;
    $nomFamilleEpouse = $_POST['nomFamilleEpouse'] ?? null;
    $details = $_POST['details'] ?? null; 

	$initialemar = substr($prenomEpouse, 0, 1) .'&'.substr($prenomEpoux, 0, 1);

    // Récupération des photos (si nécessaire)
	...
	$stmt->execute([
		...
		$type_mar,        // wedding_type
		$modeleInv,          // modele_inv
		$modeleChev,         // modele_chev
		$dateHeure,          // date_event
		$lieu,               // lieu
		$adresse,            // adresse
		$prenomEpoux,        // prenom_epoux
		$nomEpoux,           // nom_epoux
		$prenomEpouse,       // prenom_epouse
		$nomEpouse,          // nom_epouse
		$nomFamilleEpoux,    // nom_famille_epoux
		$nomFamilleEpouse,    // nom_famille_epouse 
		$nomsAnniv, //
		$themeConf, //
		$details,   // autres_precisions 
		$initialemar  
	]);

	
//-----------------------Enregistrement accessoires--------------
	$cod_event = $pdo->lastInsertId();

	$accessoires = $_POST['accessoires'] ?? []; // Récupérez les accessoires

	foreach ($accessoires as $accessoire) {
		$sqlAccessoire = "INSERT INTO accessoires_event (cod_event, cod_acc) VALUES (?, ?)";
		$stmtAccessoire = $pdo->prepare($sqlAccessoire);
		$stmtAccessoire->execute([$cod_event, $accessoire]);
	}
//-----------------------enregistrelent photos--------------
	...
	if ($photos && $photos['error'][0] === UPLOAD_ERR_OK) {
		foreach ($photos['tmp_name'] as $key => $tmpName) {
			...
			$newFileName = $initialemar . time() . '_' . $key . '_IS.' . pathinfo($photos['name'][$key], PATHINFO_EXTENSION);
			$targetPath = "photosevent/" . $newFileName; // Chemin avec le nouveau nom

			// Déplacer le fichier téléchargé
			if (move_uploaded_file($tmpName, $targetPath)) {
				// Insertion des informations de photo dans la table photos_event
				$sqlPhoto = "INSERT INTO photos_event (cod_event, nom_photo) VALUES (?, ?)";
				$stmtPhoto = $pdo->prepare($sqlPhoto);
				$stmtPhoto->execute([$cod_event, $newFileName]);
			}
		}
	}

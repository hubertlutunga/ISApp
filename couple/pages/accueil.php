...
<div id="preloader">
  ...
  <?php
    ...
    if(!$datamariage['photo']){
        $bg = 'bgstand2_1.png';
    }else{
        $bg = $datamariage['photo'];
    }
 ...
 .bg-slideshowx {
   background-image: url('images/<?php echo $bg; ?>') !important;
...
?>
						...
						<?php
							...
							<div class="overflow extra-padding-top">
								...
								<p class="center">
								    ...
								    if(!$datamariage['story']){
										$story = "Le moment tant attendu est enfin arrivé : ".$datamariage['prenom_epoux']." a pris son courage à deux mains et a demandé la main de ".$datamariage['prenom_epouse'].". Ce geste, chargé d’émotion et de symbolique, marque le début d’un nouveau chapitre dans leur histoire. La lueur dans ses yeux et le tremblement de sa voix témoignent de l’amour et de l’engagement qu’il ressent. ".$datamariage['prenom_epouse'].", surprise mais ravie, a vu en cette demande la promesse d’un avenir partagé, d’une vie construite à deux, où chaque jour sera une nouvelle aventure.";
									}else{
										$story = $datamariage['story'];
									} 
								...
								<p class="center"><a class="btn btn-primary" href="#rsvp-2">Confirmer la présence</a></p>
							</div>
					...
					<div class="col-md-12 col-lg-10 offset-lg-1 col-xl-8 offset-xl-2 center">
						<div class="invite neela-style" data-animation-direction="from-left" data-animation-delay="100">
							...
							<div class="invite_info">
								<h2><?php echo $datamariage['prenom_epoux'];?> <small>&</small> <?php echo $datamariage['prenom_epouse'];?></h2>
								
								<div class="uppercase">Demandent l'honneur de votre présence le jour de leur mariage
                                </div>
								<div class="date"><?php  echo $DateWed; ?></small></div>
    ...
    if ($datamariage['type_mar'] === 'Coutumier') {
        # code...
    }else if($datamariage['type_mar'] === 'Religieux'){
...
if(isset($_POST['submitrsvp'])){
    ...
    $q->closeCursor(); 
         ...
         // Envoi de l'email
		 $speudo = $noms;
		 $email = $email; // Assurez-vous que cette variable contient une adresse email valide
             ...
             echo '<script>
                     ...
                     confirmButtonText: "OK"
                          ...
                          if (result.isConfirmed) {
                             window.location.href = "index.php?page=accueil&cod=' . htmlspecialchars($_GET['cod']) . '"; // Rédirection vers la page de détails
                            }
                     });
             </script>';
...
?>
							...
							<form id="contact-form" class="contact-form" action="" method="post">
								...
								<div class="form-floating" style="<?php echo $display?>">
									<input type="email" name="email" id="email" value="<?php echo @$_POST['email']?>" placeholder="E-mail*" class="form-control fromEmail">
								...
								<fieldset class="form-check-wrapper" name="repas" id="meal_pref" style="<?php echo $displayx?>">

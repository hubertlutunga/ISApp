  ...
  //include('../phpqrcode/qrlib.php'); 
	...
	class myPDF extends FPDF{
      ...
      $datd = date('d');
			...
			$this->Ln(7);	
			$this->Cell(195,8,utf8_decode('('.$datm.' '.$daty.')'),0,0,'C');
			$this->Ln(5);	
			$this->SetTextColor(0, 0, 0);
			$this->SetFont('Arial','',11);		
			$this->Ln(10);	
 ...
 $prix_achat_total = "NULL";
                              ...
                              $supprimer = "non";
                               ...
                               $req="SELECT * FROM ingredien where supprimer = '$supprimer' order by nom_ing ASC";
                               $ing=$pdo->query($req);
...
$col = "qing".$q++;
      ...
      $sqlsu = "SELECT sum(su) AS total_nb from stock_utiliser where cod_ing = '{$row_ing['cod_ing']}' AND YEAR(date_su) = '$datyx' AND Month(date_su) = '$datmx' AND sorti = '$sorti' ";
      $reqsu = $pdo->query($sqlsu);  
      $row_su = $reqsu->fetch();


      $sqlsutout = "SELECT sum(su) AS total_nb from stock_utiliser where cod_ing = '{$row_ing['cod_ing']}' AND sorti = '$sorti'";

  ...
  //include('../phpqrcode/qrlib.php'); 
	...
	class myPDF extends FPDF{
		...
		function infodf($pdo){
			...
			$this->SetMargins(7,20,0,0,true);	
			$this->Cell(270,8,utf8_decode('Fournisseurs'),0,0,'C');
			$this->Ln(5);	
			$this->SetTextColor(0, 0, 0);
			$this->SetFont('Arial','',11);		
			$this->Ln(10);	
      ...
      $row_agent = $req->fetch();
			...
			$this->Cell(150,15,utf8_decode(''),0,0,'L');
			$this->Ln(0);	
			$this->Cell(60,8,utf8_decode('Entreprise'),0,0,'L',true);
			$this->Cell(56,8,utf8_decode('Chargé'),0,0,'L',true);
			$this->Cell(50,8,utf8_decode('Ville'),0,0,'L',true);
			$this->Cell(50,8,utf8_decode('E-mail'),0,0,'L',true);
			$this->Cell(50,8,utf8_decode('Mobile'),0,0,'R',true);
			$this->Ln(10);
      ...
      $supprimer = 'non';
      $sql = "SELECT * from fournisseur where supprimer = '$supprimer' order by code_fourn DESC";
      $req = $pdo->query($sql);  
      while ($row_list_fournisseur = $req->fetch()) {


			$this->SetFont('Arial','',12);	
			$this->Cell(60,8,utf8_decode($row_list_fournisseur['denom_sociale']),"B",0,'L',true);
			$this->Cell(56,8,utf8_decode($row_list_fournisseur['noms']),"B",0,'L',true);
			$this->Cell(50,8,utf8_decode($row_list_fournisseur['ville']),"B",0,'L',true);
			$this->Cell(50,8,utf8_decode($row_list_fournisseur['email']),"B",0,'L',true);
			$this->Cell(50,8,utf8_decode($row_list_fournisseur['phone']),"B",0,'R',true);
			$this->Ln(9);

  ...
  //include('../phpqrcode/qrlib.php'); 
	...
	class myPDF extends FPDF{
      ...
      $row_agent = $req->fetch();
			...
			$this->SetMargins(7,20,0,0,true);	
			$this->Cell(195,8,utf8_decode('Rapport de Ventes'),0,0,'C');
			$this->Ln(7);		
			$this->Cell(195,8,utf8_decode('Allant du '.date('d/m/Y',strtotime($_GET['da'])).' Au '.date('d/m/Y',strtotime($_GET['db']))),0,0,'C');
			$this->Ln(5);	
			$this->SetTextColor(0, 0, 0);
			$this->SetFont('Arial','',11);		
			$this->Ln(10);	
      ...
      $row_agent = $req->fetch();
			...
			$this->Cell(150,15,utf8_decode(''),0,0,'L');
			$this->Ln(0);	
			$this->Cell(75,8,'Article',0,0,'L',true);
      ...
      $sqldp = "SELECT DISTINCT cod_prod from panier_scr where  date_pan between '$da' and '$db'";
      ...
      $sqlcompqte = "SELECT sum(qte) AS total_qte from panier_scr where date_pan between '$da' and '$db' AND cod_prod = '{$row_dp['cod_prod']}'";
      $compqte = $pdo->query($sqlcompqte);  
      $row_compqte = $compqte->fetch();

      $sqlcompcout = "SELECT sum(prix_t) AS total_prix from panier_scr where date_pan between '$da' and '$db' AND cod_prod = '{$row_dp['cod_prod']}'";
...
//--------------stock recu-----------------------
      ...
      $sqlt = "SELECT sum(qte) AS total_n from stock_transfere where cod_produit = '{$row_dp['cod_prod']}' AND id_ext = '$ext' AND id_depar =  '1' AND date_enreg between '$da' and '$db' AND recu = 'oui'";
      $reqt = $pdo->query($sqlt);  
...
//------------------------------------------------
      ...
      $sqlcompqte_g = "SELECT sum(qte) AS total_qte from panier_scr where date_pan between '$da' and '$db'";
      ...
      $sqltvente = "SELECT COUNT(*) As total_vente from vente_scr WHERE date_vente between '$da' and '$db' ";
      ...
      $sqlto = "SELECT sum(prix_t) AS total_n from vente_scr where date_vente between '$da' and '$db' ";
      ...
      $sqlscr = "SELECT sum(somme_tva) AS total_sta from vente_scr where date_vente between '$da' and '$db' ";

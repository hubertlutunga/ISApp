  ...
  //include('../phpqrcode/qrlib.php'); 
	...
	class myPDF extends FPDF{
      ...
      $row_agent = $req->fetch();
			...
			$this->SetMargins(7,20,0,0,true);	
			$this->Cell(195,8,utf8_decode('Rapport Mensuel ( '.date('m/Y').' )'),0,0,'C');
			$this->Ln(5);	
			$this->SetTextColor(0, 0, 0);
			$this->SetFont('Arial','',11);		
			$this->Ln(10);	
      ...
      $sqldp = "SELECT DISTINCT cod_prod from panier_scr where YEAR(date_pan) = '$daty' AND Month(date_pan) = '$datm'";
      ...
      $sqlcompqte = "SELECT sum(qte) AS total_qte from panier_scr where YEAR(date_pan) = '$daty' AND Month(date_pan) = '$datm' AND cod_prod = '{$row_dp['cod_prod']}'";
      $compqte = $pdo->query($sqlcompqte);  
      $row_compqte = $compqte->fetch();

      $sqlcompcout = "SELECT sum(prix_t) AS total_prix from panier_scr where YEAR(date_pan) = '$daty' AND Month(date_pan) = '$datm' AND cod_prod = '{$row_dp['cod_prod']}'";
...
//--------------stock recu-----------------------
      ...
      $sqlt = "SELECT sum(qte) AS total_n from stock_transfere where cod_produit = '{$row_dp['cod_prod']}' AND id_ext = '$ext' AND id_depar =  '1' AND YEAR(date_enreg) = '$daty' AND Month(date_enreg) = '$datm' AND recu = 'oui'";
      $reqt = $pdo->query($sqlt);  
...
//------------------------------------------------
      ...
      $sqlcompqte_g = "SELECT sum(qte) AS total_qte from panier_scr where YEAR(date_pan) = '$daty' AND Month(date_pan) = '$datm'";
      ...
      $sqltvente = "SELECT COUNT(*) As total_vente from vente_scr WHERE YEAR(date_vente) = '$daty' AND Month(date_vente) = '$datm' ";
      ...
      $sqlto = "SELECT sum(prix_t) AS total_n from vente_scr where YEAR(date_vente) = '$daty' AND Month(date_vente) = '$datm'";
      ...
      $sqlscr = "SELECT sum(somme_tva) AS total_sta from vente_scr where YEAR(date_vente) = '$daty' AND Month(date_vente) = '$datm'";

...
<?php
$confSiteGalleryAjaxUpload = strtolower((string) ($_SERVER['HTTP_X_REQUESTED_WITH'] ?? '')) === 'xmlhttprequest'
  && isset($_POST['submit_wedding_gallery_upload']);
if ($confSiteGalleryAjaxUpload) {
  ...
  $sessionStmt = $pdo->prepare('SELECT * FROM is_users WHERE phone = ?');
  $sessionStmt->execute([$_SESSION['user_phone']]);
  $datasession = $sessionStmt->fetch(PDO::FETCH_ASSOC) ?: [];

  $eventContext = EventWorkspaceService::resolveCurrentEventContext($pdo, $datasession, $_GET);
  extract($eventContext, EXTR_OVERWRITE);

  if (empty($codevent) || (string) ($type_event ?? '') !== '1') {
    http_response_code(404);
    header('Content-Type: application/json; charset=utf-8');
    echo json_encode([
      'success' => false,
      'message' => 'Événement introuvable pour l’importation de la galerie.',
    ], JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
    return;
  }

  include('editersite.php');
  return;
}
?>
...
$weddingSiteFieldLabels = [
  ...
  'home2_hero_text_color' => 'Couleur texte Accueil',
  'home2_gala_text_color' => 'Couleur texte Soirée',
  'home2_program_text_color' => 'Couleur texte Programme',
  'home2_story_text_color' => 'Couleur texte Histoire',
  'home2_presence_text_color' => 'Couleur texte Présences',
  'home2_location_text_color' => 'Couleur texte Adresse',
  'home2_rsvp_text_color' => 'Couleur texte RSVP',
  'home2_footer_text_color' => 'Couleur texte Footer',
];
...
.mb-wedconf-logo-choice{ position:relative; display:inline-flex; align-items:center; justify-content:center; min-height:44px; padding:0 16px; border:1px solid #cbd5e1; border-radius:14px; background:#fff; color:#334155; cursor:pointer; transition:all .18s ease; box-shadow:0 8px 20px rgba(15,23,42,.05); }
.mb-wedconf-logo-choice input{ position:absolute; opacity:0; pointer-events:none; }
.mb-wedconf-logo-choice span{ display:inline-flex; align-items:center; gap:7px; font-size:13px; font-weight:900; }
.mb-wedconf-logo-choice:has(input:checked){ border-color:#14b8a6; background:linear-gradient(135deg,#ecfeff 0%,#ffffff 100%); color:#0f766e; box-shadow:0 12px 26px rgba(20,184,166,.14); }
.mb-wedconf-logo-choice:has(input:checked) span::before{ content:'✓'; display:inline-flex; align-items:center; justify-content:center; width:18px; height:18px; border-radius:999px; background:#14b8a6; color:#fff; font-size:12px; font-weight:900; }
...
.mb-wedconf-progress{ display:none; margin-top:14px; }
.mb-wedconf-progress.is-visible{ display:block; }
.mb-wedconf-progress-track{ width:100%; height:16px; overflow:hidden; border-radius:999px; background:#e2e8f0; }
.mb-wedconf-progress-bar{ width:0; height:100%; background:linear-gradient(135deg,#0f766e 0%,#14b8a6 100%); transition:width .2s ease; }
.mb-wedconf-progress-text{ display:block; margin-top:8px; color:#334155; font-size:13px; font-weight:800; }
...
.mb-wedconf-section-card.is-section-hidden{ opacity:.72; }
.mb-wedconf-section-card.is-section-hidden .mb-wedconf-section-body{ display:none; }
              ...
              <div class="box-body">
                <form action="" method="post" enctype="multipart/form-data" id="weddingSiteForm">
                  ...
                  <div class="mb-wedconf-style-panel <?php echo $selectedHomeTemplate === 'home2' ? 'is-visible' : ''; ?>" data-style-panel="home2">
                    <div class="alert alert-light" style="border-radius:16px;font-weight:700;">Ces champs pilotent directement le Style 2 : textes, programme, adresse, RSVP, carte, monogramme, backgrounds et couleurs de texte des sections. Pour les couleurs, utilisez un code hexadécimal comme #876987.</div>
                    ...
                    <div class="mb-wedconf-section-card">
                      ...
                      <div class="mb-wedconf-section-body">
                        ...
                        <div class="mb-wedconf-grid">
                          ...
                          <div class="mb-wedconf-field"><label for="wed_<?php echo htmlspecialchars($fieldName, ENT_QUOTES, 'UTF-8'); ?>"><?php echo htmlspecialchars($weddingSiteFieldLabels[$fieldName] ?? $fieldName, ENT_QUOTES, 'UTF-8'); ?></label><?php if ($isLogoModeField) { $modeValue = in_array($fieldValue, ['text', 'image'], true) ? $fieldValue : 'text'; ?><div class="mb-wedconf-radio-group" data-home2-mode-group="<?php echo htmlspecialchars($fieldName, ENT_QUOTES, 'UTF-8'); ?>"><label class="mb-wedconf-logo-choice"><input type="radio" id="wed_<?php echo htmlspecialchars($fieldName, ENT_QUOTES, 'UTF-8'); ?>_text" name="<?php echo htmlspecialchars($fieldName, ENT_QUOTES, 'UTF-8'); ?>" value="text" <?php echo $modeValue === 'text' ? 'checked' : ''; ?>><span>Texte</span></label><label class="mb-wedconf-logo-choice"><input type="radio" id="wed_<?php echo htmlspecialchars($fieldName, ENT_QUOTES, 'UTF-8'); ?>_image" name="<?php echo htmlspecialchars($fieldName, ENT_QUOTES, 'UTF-8'); ?>" value="image" <?php echo $modeValue === 'image' ? 'checked' : ''; ?>><span>Image importée</span></label></div><small style="display:block;margin-top:8px;color:#64748b;font-weight:700;">Cliquez sur un bouton pour activer le texte ou l’image importée. Si vous choisissez un fichier image, le mode Image importée s’active automatiquement.</small><?php } elseif ($isLongField) { ?><textarea id="wed_<?php echo htmlspecialchars($fieldName, ENT_QUOTES, 'UTF-8'); ?>" name="<?php echo htmlspecialchars($fieldName, ENT_QUOTES, 'UTF-8'); ?>" class="form-control" rows="4"><?php echo htmlspecialchars($fieldValue, ENT_QUOTES, 'UTF-8'); ?></textarea><?php } elseif ($isColorField) { ?><input id="wed_<?php echo htmlspecialchars($fieldName, ENT_QUOTES, 'UTF-8'); ?>" type="text" name="<?php echo htmlspecialchars($fieldName, ENT_QUOTES, 'UTF-8'); ?>" class="form-control mb-wedconf-color-input" value="<?php echo htmlspecialchars($fieldValue, ENT_QUOTES, 'UTF-8'); ?>" placeholder="#876987" maxlength="7" pattern="^#([0-9A-Fa-f]{3}|[0-9A-Fa-f]{6})$" title="Format attendu : #876987 ou #fff"><?php } else { ?><input id="wed_<?php echo htmlspecialchars($fieldName, ENT_QUOTES, 'UTF-8'); ?>" type="text" name="<?php echo htmlspecialchars($fieldName, ENT_QUOTES, 'UTF-8'); ?>" class="form-control" value="<?php echo htmlspecialchars($fieldValue, ENT_QUOTES, 'UTF-8'); ?>"><?php } ?></div>
                          <?php } ?>
                        </div>
                        <?php } ?>
                        ...
                        <div class="mb-wedconf-image-grid" style="margin-top:12px;">
                          <?php foreach ($style2Panel['images'] as $imageField) { $imageUrl = $weddingSiteImageUrl((string) ($weddingSiteSettings['images'][$imageField] ?? '')); $autoModeField = ['home2_header_logo_image' => 'home2_header_logo_mode', 'home2_hero_names_image' => 'home2_hero_names_mode'][$imageField] ?? ''; ?>
                          <div class="mb-wedconf-image-card"><strong><?php echo htmlspecialchars($weddingSiteImageFields[$imageField] ?? $imageField, ENT_QUOTES, 'UTF-8'); ?></strong><?php if ($imageUrl !== '') { ?><img src="<?php echo $imageUrl; ?>" alt="<?php echo htmlspecialchars($weddingSiteImageFields[$imageField] ?? $imageField, ENT_QUOTES, 'UTF-8'); ?>"><?php } else { ?><div class="mb-wedconf-empty-img">Image par défaut</div><?php } ?><input type="file" name="wedding_image_<?php echo htmlspecialchars($imageField, ENT_QUOTES, 'UTF-8'); ?>" accept="image/*" class="form-control" <?php echo $autoModeField !== '' ? 'data-home2-auto-mode="' . htmlspecialchars($autoModeField, ENT_QUOTES, 'UTF-8') . '"' : ''; ?>><?php if ($autoModeField !== '') { ?><small style="display:block;margin-top:8px;color:#64748b;font-weight:800;">Choisir un fichier active automatiquement “Image importée”.</small><?php } ?></div>
                          <?php } ?>
                        </div>
                        <?php } ?>
                      </div>
                  ...
                  <div class="mb-wedconf-modal" id="galleryModal" aria-hidden="true"><div class="mb-wedconf-modal-card"><div class="mb-wedconf-modal-head"><h5>Importer des photos dans la galerie</h5><button type="button" class="btn btn-warning btn-sm" id="closeGalleryModal">Fermer</button></div><div class="alert alert-light" style="border-radius:16px;font-weight:700;">Sélectionnez les photos à ajouter, puis enregistrez. La galerie est limitée à 5 photos.</div><div class="mb-wedconf-field mb-0"><label for="fileInput">Photos de la galerie</label><input type="file" name="gallery_photos[]" class="form-control" accept="image/*" id="fileInput" multiple <?php echo $galleryRemainingSlots <= 0 ? 'disabled' : ''; ?> data-max-files="<?php echo (int) $galleryRemainingSlots; ?>"><small style="display:block;margin-top:8px;color:#64748b;font-weight:700;">Places disponibles : <?php echo (int) $galleryRemainingSlots; ?> photo(s).</small><div id="previewContainer" class="mt-2" style="display:flex;flex-wrap:wrap;"></div></div><div id="galleryUploadProgress" class="mb-wedconf-progress"><div class="mb-wedconf-progress-track"><div id="galleryUploadProgressBar" class="mb-wedconf-progress-bar"></div></div><span id="galleryUploadProgressText" class="mb-wedconf-progress-text">Téléchargement des photos : 0%</span></div><div class="mt-15"><button type="button" id="submitGalleryUpload" class="btn btn-success w-p100" <?php echo $galleryRemainingSlots <= 0 ? 'disabled' : ''; ?>>Enregistrer les photos de la galerie</button></div></div></div>
...
(function(){ document.querySelectorAll('[data-home2-auto-mode]').forEach(function(input){ input.addEventListener('change', function(){ if (!input.files || !input.files.length) return; const fieldName = input.getAttribute('data-home2-auto-mode'); const radio = document.querySelector('input[type="radio"][name="' + fieldName + '"][value="image"]'); if (!radio) return; radio.checked = true; radio.dispatchEvent(new Event('change', {bubbles:true})); }); }); })();
...
document.addEventListener('DOMContentLoaded', function(){ const input = document.getElementById('fileInput'); const preview = document.getElementById('previewContainer'); const submitBtn = document.getElementById('submitGalleryUpload'); const progress = document.getElementById('galleryUploadProgress'); const progressBar = document.getElementById('galleryUploadProgressBar'); const progressText = document.getElementById('galleryUploadProgressText'); const closeBtn = document.getElementById('closeGalleryModal'); const galleryModal = document.getElementById('galleryModal'); const uploadUrl = 'index.php?page=conf_siteweb&codevent=<?php echo (int) $codevent; ?>'; if (input && preview) { input.addEventListener('change', function(){ preview.innerHTML = ''; const maxFiles = parseInt(input.getAttribute('data-max-files') || '5', 10); const files = Array.from(input.files || []); if (files.length > maxFiles) { Swal.fire({icon:'warning', title:'Limite atteinte', text:'La galerie ne peut pas dépasser 5 photos.'}); input.value = ''; return; } files.forEach(function(file){ if (!file.type.startsWith('image/')) return; const reader = new FileReader(); reader.onload = function(e){ const img = document.createElement('img'); img.src = e.target.result; img.style.width = '70px'; img.style.height = '70px'; img.style.objectFit = 'cover'; img.style.borderRadius = '10px'; img.style.marginRight = '8px'; img.style.marginTop = '8px'; preview.appendChild(img); }; reader.readAsDataURL(file); }); }); } if (!input || !submitBtn || !progress || !progressBar || !progressText) return; submitBtn.addEventListener('click', function(){ const files = Array.from(input.files || []); if (!files.length) { Swal.fire({icon:'warning', title:'Aucune photo', text:'Sélectionnez au moins une photo à importer.'}); return; } const formData = new FormData(); files.forEach(function(file){ formData.append('gallery_photos[]', file); }); formData.append('submit_wedding_gallery_upload', '1'); formData.append('codevent', '<?php echo (int) $codevent; ?>'); const xhr = new XMLHttpRequest(); xhr.open('POST', uploadUrl, true); xhr.setRequestHeader('X-Requested-With', 'XMLHttpRequest'); progress.classList.add('is-visible'); progressBar.style.width = '0%'; progressText.textContent = 'Téléchargement des photos : 0%'; submitBtn.disabled = true; if (closeBtn) closeBtn.disabled = true; xhr.upload.onprogress = function(event){ if (!event.lengthComputable) return; const percent = Math.round((event.loaded / event.total) * 100); progressBar.style.width = percent + '%'; progressText.textContent = 'Téléchargement des photos : ' + percent + '%'; }; xhr.onload = function(){ let payload = null; try { payload = JSON.parse(xhr.responseText); } catch (error) { payload = null; } if (xhr.status === 200 && payload && payload.success) { progressBar.style.width = '100%'; progressText.textContent = 'Téléchargement des photos : 100%'; if (galleryModal) { galleryModal.classList.remove('is-open'); galleryModal.setAttribute('aria-hidden', 'true'); } setTimeout(function(){ window.location.href = payload.redirect || uploadUrl + '&okwedding=1'; }, 250); return; } const fallbackMessage = xhr.responseText ? xhr.responseText.replace(/\s+/g, ' ').trim().slice(0, 220) : ''; const message = payload && payload.message ? payload.message : (fallbackMessage || 'Impossible d’importer les photos de la galerie.'); progress.classList.remove('is-visible'); submitBtn.disabled = false; if (closeBtn) closeBtn.disabled = false; Swal.fire({icon:'error', title:'Importation impossible', text:message}); }; xhr.onerror = function(){ progress.classList.remove('is-visible'); submitBtn.disabled = false; if (closeBtn) closeBtn.disabled = false; Swal.fire({icon:'error', title:'Importation impossible', text:'Une erreur réseau est survenue pendant l’envoi des photos.'}); }; xhr.send(formData); }); if (galleryModal) { galleryModal.addEventListener('click', function(event){ if (event.target === galleryModal && submitBtn.disabled) event.stopPropagation(); }); } });
</script>

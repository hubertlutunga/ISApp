   ...
   <!-- 				
						   ...
						   <div class="form-group"> 
							   <div class="input-group date">
								   ...
								   <input type="datetime-local" value="<?php echo $date_formatted; ?>" class="form-control pull-right" id="datepicker">
								   <button type="button" class="btn btn-info btn-sm">Afficher</button>
						   ...
						   <div class="col-md-12"> 
							   ...
							   if ($dataevent) {
								   ...
								   $typmaract = isset($dataevent['type_mar']) ? $dataevent['type_mar'] : '';
							   ...
							   <option value="#"><?php echo $data_evenement.' '.$typmaract.', le '.date('d/m/Y à H:i',strtotime($date_event))?></option>
	
								
   
							   <?php 
							   $reqevget = $pdo->prepare("SELECT * FROM events where cod_event != ? AND (cod_user = ? OR cod_user2 = ?) ORDER by date_event DESC");
							   $reqevget->execute([$vuoption, $datasession['cod_user'], $datasession['cod_user']]);  
							   while ($data_evget = $reqevget->fetch()) {
   
								   if ($data_evget['type_event'] == '1') {
									   $typmar = ' '.$data_evget['type_mar']; 
								   }else{
									   $typmar = '';
								   }
					   
   
							   $stmtnvg = $pdo->prepare("SELECT * FROM evenement WHERE cod_event = ?");
							   $stmtnvg->execute([$data_evget['type_event']]); // Correction ici pour utiliser $codevent
							   $data_eventget = $stmtnvg->fetch();
   
							   ?>
							   <option value="index.php?page=mb_accueil&codevent=<?php echo $data_evget['cod_event']?>" ><?php echo $data_eventget['nom'].$typmar.', le '.date('d/m/Y à H:i',strtotime($data_evget['date_event']))?></option>
							   <?php } ?>  
							   ...
							   <?php
									   ...
									   ?>
									   <option value="#">Aucun événement trouvé</option>
									   <?php 
								   }
				   ...
				   <br>
					   ...
					   <div class="row">
						...
						<?php include('calendrier_event.php')?>

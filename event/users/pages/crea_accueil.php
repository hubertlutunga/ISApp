...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  $codevent = (int) ($_POST['codevent'] ?? 0);
  $observation = (string) ($_POST['observation'] ?? '');
  $user = (int) ($datasession['cod_user'] ?? 0);
  ...
  $uploadSuccess = EventPrintService::finalizeCreation(
    ...
    // Afficher le message SweetAlert
    if ($uploadSuccess) {
        echo '<script>
        Swal.fire({
            title: "Rapport",
            ...
            if (result.isConfirmed) {
                window.location.href = "index.php?page=crea_accueil"; // Rédirection vers la page de détails
            }
        });
        </script>';
    } else {
        ...
        Swal.fire({
            title: "Erreur",
            text: "Le téléchargement a échoué. Veuillez réessayer.",
            icon: "error",
        ...
        <form id="eventForm" action="" method="post" enctype="multipart/form-data">
            ...
            <div class="row"> 
                <div class="col-12 text-center">
                    <button type="submit" name="submit" class="btn btn-primary w-p100 mt-10">Enregistrer</button>
                </div>
            </div>
        </form>
    </div>
    ...
    xhr.onload = function() {
        if (xhr.status === 200) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = "index.php?page=crea_accueil"; // Redirection
                }
            });
        } else {

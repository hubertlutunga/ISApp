...
//----------------------------------------------------------------------
      ...
      $sqlc = "SELECT * from commandes  where date_enreg between '$da' and '$db' AND code_cli = '$codcli'";
      $reqc = $pdo->query($sqlc);  
      $row_com = $reqc->fetch();

      $sqls = "SELECT COUNT(*) As total_op from commandes where date_enreg between '$da' and '$db' AND code_cli = '$codcli'";
      $reqs = $pdo->query($sqls);  
      ...
      $sqlts = "SELECT sum(pvt) As total_ts from commandes where date_enreg between '$da' and '$db' AND code_cli = '$codcli'";
      $ts = $pdo->query($sqlts);  
      $row_somts = $ts->fetch();
      ...
      $sqlsr = "SELECT sum(somme_remise) As total_sr from commandes where date_enreg between '$da' and '$db' AND code_cli = '$codcli'";
      $sr = $pdo->query($sqlsr);  
      $row_somrem = $sr->fetch();
      ...
      $somme_remise = $row_somrem['total_sr'];
			...
			$this->Cell(43,8,utf8_decode('Periode'),0,0,'L',true);
      ...
      $sqlx = "SELECT * from paiement_remise WHERE date_paie between '$da' and '$db' AND matr_cli = '{$_GET['cod']}' order by cod_pr DESC LIMIT 100";
      $reqx = $pdo->query($sqlx);  
      ...
      $sqlcp = "SELECT * from paiement_remise WHERE matr_cli = '{$_GET['cod']}' order by cod_pr DESC LIMIT 2";
      $reqcp = $pdo->query($sqlcp);  
      $row_cp = $reqcp->fetch();

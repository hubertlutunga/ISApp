	...
	class myPDF extends FPDF{
		...
		function infodf($pdo){
			...
			$this->Ln(12);			
					...
					if ($class == 'PAT') {
						$nclass = "Demande de stock Patisserie";
					}elseif($class == 'BOUL'){
							$nclass = "Demande de stock Boulangerie";
						}elseif ($class == 'CUIS') {
								$nclass = "Demande de stock Cuisine";
							}else{
									$nclass = "Demande de stock";
									}
			...
			$this->SetMargins(7,20,0,0,true);		
			$this->Cell(195,8,$nclass.' du '.date('d/m/Y H:i',strtotime($_GET['da'])).' au '.date('d/m/Y H:i',strtotime($_GET['db'])),0,0,'C');
			$this->Ln(5);	
			$this->SetTextColor(0, 0, 0);
			$this->SetFont('Arial','',11);		
			$this->Ln(10);	
	 ...
	 $sqlistv = "SELECT * FROM production WHERE rapport = :rapport AND date_production BETWEEN :da AND :db $cond ORDER BY code_production DESC";
	 $stmt = $pdo->prepare($sqlistv);
	 ...
	 while ($row_list_product = $stmt->fetch()){
...
*/
	  ...
	  if ($row_ing_li['cat_ing'] == 'ING' || $row_ing_li['cat_ing'] === NULL) {
		$rowingli = $row_ing_li['nom_ing'] . ' (' . $rowmomarpst . ')';
		   ...
		   if ($class == 'PAT') {
			$condag = "AND classe = 'PAT'";
		} elseif ($class == 'BOUL') {
			$condag = "AND classe = 'BOUL'";
		} elseif ($class == 'CUIS') {
			$condag = "AND classe = 'RES'";
		} else {
			$condag = "";
		}
		
		$sqlistvag = "SELECT * FROM production WHERE rapport = :rapport AND date_production BETWEEN :da AND :db $condag ORDER BY code_production DESC LIMIT 1";
		$stmt = $pdo->prepare($sqlistvag);
		...
		$row_list_productag = $stmt->fetch();



	  if ($row_list_productag) {
		$rowlistproductag = $row_list_productag['code_agent'];
		} else {
			$rowlistproductag = 'NULL';
		}
      ...
      $sqlag = "SELECT * from abc_user WHERE matr_user = '$rowlistproductag'";
      $reqag = $pdo->query($sqlag);  
      $agent = $reqag->fetch();


	  if ($agent) {
		$agenttire = $agent['prenom'].' '.$agent['nom'];
		} else {
            $agenttire = 'Prenom Nom';
		}
           ...
           $this->Cell(190,8,'','B',1,'L',true);
           $this->Ln(4);	
           $this->Cell(95,8,'Gestionnaire de stock',0,0,'L',true);
           $this->Cell(95,8,$ag,0,0,'R',true);
           $this->SetFont('Arial','',14);	
           $this->Ln(8);	
           $this->Cell(95,8,$ussession['prenom'].' '.$ussession['nom'],0,0,'L',true);
           $this->Cell(95,8,$agenttire,0,0,'R',true);
           $this->Ln(15);
           $this->Cell(95,8,'Signature',0,0,'L',true);
           $this->Cell(95,8,'Signature',0,0,'R',true);
           $this->Ln(7);

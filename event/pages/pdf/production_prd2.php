  ...
  //include('../phpqrcode/qrlib.php'); 
	...
	class myPDF extends FPDF{
		...
		function infodf($pdo){
			...
			$this->SetMargins(7,20,0,0,true);		
			$this->Cell(195,8,utf8_decode('Productions du '.date('d M Y',strtotime($_GET['da'])).' au '.date('d M Y',strtotime($_GET['db']))),0,0,'C');
			$this->Ln(5);	
			$this->SetTextColor(0, 0, 0);
			$this->SetFont('Arial','',11);		
			$this->Ln(10);	
	 ...
	 $raison = "production";
      ...
      $sqlistv = "SELECT * from production where date_production between '$da' and '$db'  order by code_production DESC";
...
*/
      ...
      $sqlli = "SELECT DISTINCT  cod_ing,cod_mp from stock_utiliser where  date_su between '$da' and '$db' AND raison = '$raison' order by cod_ing ASC";
      ...
      $sqlto = "SELECT sum(su) AS total_n from stock_utiliser where cod_ing = '$cod_ing' AND cod_mp = '$marq' AND date_su between '$da' and '$db'";

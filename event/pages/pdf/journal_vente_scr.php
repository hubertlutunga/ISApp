  ...
  //include('../phpqrcode/qrlib.php'); 
	...
	class myPDF extends FPDF{
		...
		function infodf($pdo){
			...
			$this->SetMargins(7,20,0,0,true);	
			$this->Cell(195,8,utf8_decode('Journal des ventes ('.date('M Y').')'),0,0,'C');
			$this->Ln(5);	
			$this->SetTextColor(0, 0, 0);
			$this->SetFont('Arial','',11);		
			$this->Ln(10);	
      ...
      $datm = date('m');
      $datd = date('d');
      $sqlistv = "SELECT * from vente_scr WHERE YEAR(date_vente) = '$daty' AND Month(date_vente) = '$datm' AND agent = '{$row_agent['matr_user']}' order by cod_vente DESC";
      ...
      $row_prod = $reqs->fetch();
			...
			$this->Cell(50,8,utf8_decode(number_format($row_dp['prix_u'], 2, '.', ' ').' CDF'),1,0,'R',true);
			$this->Cell(50,8,utf8_decode(number_format($row_dp['prix_t'], 2, '.', ' ').' CDF'),1,0,'R',true);
			$this->Ln(7);
      ...
      $sqltvente = "SELECT COUNT(*) As total_vente from vente_scr WHERE YEAR(date_vente) = '$daty' AND Month(date_vente) = '$datm' AND agent = '{$row_agent['matr_user']}'";
      ...
      $sqlto = "SELECT sum(prix_t) AS total_n from vente_scr where YEAR(date_vente) = '$daty' AND Month(date_vente) = '$datm' AND agent = '{$row_agent['matr_user']}'";
      ...
      $sqlscr = "SELECT sum(somme_tva) AS total_sta from vente_scr where YEAR(date_vente) = '$daty' AND Month(date_vente) = '$datm' AND agent = '{$row_agent['matr_user']}'";
      ...
      $total_a_payer = $cal_total_santtva + $total_tva;
			...
			$this->Ln(7);	
			$this->Cell(185,8,'Montant total des ventes : '.number_format($row_prixto['total_n'], 2, '.', ' ').' CDF',0,0,'R',true);
			$this->Ln(7);
			$this->Cell(185,8,'Hors taxes : '.number_format($cal_total_santtva, 2, '.', ' ').' $',0,0,'R',true);
			$this->Ln(7);
			$this->Cell(185,8,'TVA ('.$row_tvaf['pourc_tva'].'%) : '.number_format($rec_tva, 2, '.', ' ').' CDF',0,0,'R',true);
			$this->Ln(7);

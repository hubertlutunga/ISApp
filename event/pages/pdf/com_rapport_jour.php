  ...
  //include('../phpqrcode/qrlib.php'); 
	...
	class myPDF extends FPDF{
      ...
      $sqlint = "SELECT * from interface where cod_inte = '{$row_agent['cod_type_user']}'";
      $reqint = $pdo->query($sqlint);  
      $row_int = $reqint->fetch();

      $ext = $row_agent['extension'];
...
//--------------stock recu-----------------------
      ...
      $sqlt = "SELECT sum(qte) AS total_n from stock_transfere where cod_produit = '{$row_list_com['code_prodvente']}' AND id_depar =  '{$row_int['id_depar']}' AND date_enreg  between '$ha' and '$hb' AND recu = 'oui'";
      $reqt = $pdo->query($sqlt);  

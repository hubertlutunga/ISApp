...
<div id="preloader">
		<div class="loading-heart">
			...
			<small>20 ans de Mariage</small><br>
            <?php echo $dataan['noms'];?>
			</div>
		</div>
	</div>
	<!-- END PRELOADER -->
		...
		<header id="header">
			<div class="nav-section">
				<div class="container">
					<div class="row">
						<div class="col-sm-12">
                        <a style="font-family: 'Great Vibes', cursive;font-size:40px;margin-top:-2px;" href="index.php?page=accueil&cod=<?php echo $codan?>" class="nav-logo"><?php echo $dataan['logo_texte'];?></a></a>
 ...
 .bg-slideshowx {
   background-image: url('images/<?php echo $dataan['photocover']; ?>');
   ...
   height: 100vh;
		...
		<section id="the-wedding" class="parallax-background bg-color-overlay padding-divider-top">
			...
			<?php
			                  ...
			                  $reqcnt="SELECT count(*) as total_n FROM confirmation where cod_mar = '{$_GET['cod']}'";
                              $cnt=$pdo->query($reqcnt);
                              $row_cnt=$cnt->fetch();
                              
                              if($row_cnt['total_n'] < 2){
                                  $title = $row_cnt['total_n']. ' invité confirmé';
                              }else{
                                  $title = $row_cnt['total_n']. ' invités confirmés';
                              }
                              
            ?>		    
						<h2 class="section-title light"><br><br><?php echo $title; ?></h2>
					</div>
				</div>

...
<div id="shareModal" class="modalinv">
  <div class="modal-content">
    <span class="close" onclick="closeVernissageConfirmModal()" aria-label="Fermer">&times;</span>
    ...
    <div id="modalInfoContainer" style="display:none;">
      ...
      <div class="text-right">
        <button type="button" class="btn btn-primary" onclick="closeVernissageConfirmModal()">OK</button>
      </div>
    </div>
    ...
    <div id="modalFormContainer" style="display:none;">
      <form id="confirmForm" method="post"
        ...
        <input type="hidden" id="cod_mar" name="cod_mar" value="<?php echo htmlspecialchars((string) ($_GET['cod'] ?? ''), ENT_QUOTES, 'UTF-8') ?>">
        <input type="hidden" id="idinv" name="idinv" value="<?php echo htmlspecialchars((string) ($_GET['idinv'] ?? ''), ENT_QUOTES, 'UTF-8') ?>">
        ...
        <button class="btn btn-primary" type="button" id="btnSubConf"
                style="width:100%;"
                onclick="submitVernissageConfirmForm()">Confirmer</button>
      </form>
    </div>
  </div>
</div>
...
<script>
function openVernissageConfirmModal(inviteName, codevent, idinv, mode){
...
function closeVernissageConfirmModal(){
  document.getElementById('shareModal').classList.remove('show');
}

function submitVernissageConfirmForm(){
